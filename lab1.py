...
class Automata():
    def __init__(self,expr):
        ...
        self.nfaSemEpsilon ={0:{expression:1}}
    ...
    def generateFormat(self):
        for origem, caminho in self.nfaSemEpsilon.items():
            if origem in self.estadosFinais:
                self.graph.node(str(origem),str(origem), shape='doublecircle')
            else: self.graph.node(str(origem),str(origem), shape='circle')
        self.graph.node('1','1', shape='doublecircle')
        for origem, caminho in self.nfaSemEpsilon.items():
            for transicao,destino in self.nfaSemEpsilon[origem].items():
                for node in destino:
                    if transicao == 'e':
                        self.graph.edge(str(origem),str(node), label = '&')
                    else: self.graph.edge(str(origem),str(node), label = transicao)
